    ...
    <div class="container pd-x-0">
      <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
        ...
        <div class="d-md-block">
          <a href="{{ route('admin.list_cse') }}" class="btn btn-primary"><i data-feather="wind"></i> CSE List</a>
        </div>
      </div>

      <form method="POST" action="{{ route('admin.store_cse') }}" enctype="multipart/form-data">
        @csrf
          ...
          <div class="col-md-12">
                  ...
                  @foreach ($services as $service)
                    <optgroup label="{{ $service->name }}">
                      @foreach($service->categories as $item)
                        <option value="{{ $item->id }}" {{ old('cse_category') == $item->id ? 'selected' : ''}}>{{ $item->name }}</option>
                      @endforeach
                    </optgroup>
                  @endforeach
                </select>
            ...
            <div class="form-row">
              <div class="form-group col-md-12">
                ...
                <textarea class="form-control @error('full_address') is-invalid @enderror" rows="3" name="full_address" id="full_address" placeholder="e.g. 284B, Ajose Adeogun Street, Victoria Island, Lagos, Nigeria.">{{ old('full_address') }}</textarea>
                @error('full_address')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
