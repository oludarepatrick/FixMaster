...
<div class="content-body">
    <div class="container pd-x-0">
      ...
      <form method="POST" action="{{ route('admin.store_technician') }}" enctype="multipart/form-data">
        @csrf
                  ...
                  @foreach ($services as $service)
                    <optgroup label="{{ $service->name }}">
                      @foreach($service->categories as $item)
                        <option value="{{ $item->id }}" {{ old('technician_category') == $item->id ? 'selected' : ''}}>{{ $item->name }}</option>
                      @endforeach
                    </optgroup>
                  @endforeach
                </select>
