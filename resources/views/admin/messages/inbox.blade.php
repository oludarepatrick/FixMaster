    ...
    <div class="mail-group-body">
      ...
      <label class="mail-group-label">{{ $message }}</label>
        <ul class="list-unstyled media-list mg-b-0">
          ...
          <li class="media @if($value->is_read == '0') unread @endif" data-url="{{ route('admin.inbox_message_details', $value->id) }}">
  ...
  $(document).ready(function (){
    ...
    $(document).on('change', '#ongoing_requests', function () {
        ...
        $.ajax({
            ...
            error: function(jqXHR, testStatus, error) {
                var message = error+ ' occured while trying to retireve '+ user +' detail.';
                var type = 'error';
                displayMessage(message, type);
                $("#spinner-icon-admin").hide();
            },
